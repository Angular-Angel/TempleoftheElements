...
class FighterGenerator implements GenerationProcedure<CreatureDefinition> {
    ...
    public CreatureDefinition generate() {
        ...
        statTotal -= tempStat;
        ret.addStat("Constitution", new NumericStat(10 + tempStat)); //generate the core attributes
        tempStat = random.nextInt(statTotal);
        statTotal -= tempStat;
        ret.addStat("Dexterity", new NumericStat(10 + tempStat));
        tempStat = random.nextInt(statTotal);
        statTotal -= tempStat;
        ret.addStat("Strength", new NumericStat(12 + tempStat));
        tempStat = random.nextInt(statTotal);
        statTotal -= tempStat;
